...
An Administrator must set up the interface before it appears in Visual KPI Designer. For more information on how to set up anm interface, go to the [Admin guide for Interfaces Configuration](/docs/setup-and-administration/interfaces/configuration).
...
Use the Site Settings icon (<PngFont name='options28' />) to access Advanced/Nav Bar Links. Add the **Name** and a **Description** (optional), copy the **URL**, and use the pull-down menu to Show **TRUE**. You can change this attribute to FALSE if you don't want the link to display later.
...
Designers can use Visual KPI Designer to modify site rollups by navigating to the Site Settings, Advanced, and Attributes sections. The **Site Rollup** name, **Description** and **URL** are the attributes used to identify each Rollup.

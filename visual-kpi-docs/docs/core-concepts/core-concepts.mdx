...
- **[Hierarchy/Groups](./core-concepts/hierarchy)**: Hierarchy and Groups are the organizational structures of Visual KPI. They act like organized folders containing KPIs, Charts, and Tables. They are almost synonyms, with the difference being that different Groups can have the same KPI. Hierarchy is a way to categorize your data. It automatically shows data in various colors to indicate its status so that you can easily see the percentage of KPIs in different states. Groups summarize the status of all elements inside them.

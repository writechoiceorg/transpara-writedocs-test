...
As a result, KPIs and groups that are not part of a user's access level won't appear on the screen. Consequently, user groups with different access levels can access the same dashboard but view different KPIs in their browser.

...
- An installed version of [Visual KPI Designer](/docs/visual-kpi-design/installation).
...
To ensure accurate data representation on the KPI dashboard, it's recommended to modify the default setting for **KPI Stale Age**. The default setting is 60 minutes, which implies that when data retrieved via an Interface (such as truck or mine metrics) is older than an hour (timestamp), the KPI status will change to Stale. Therefore, it's advisable to adjust this setting to better suit your data requirements and ensure that the KPIs reflect the latest data.
...
In this step, it's recommended to use SQL Server Management Studio to create a database for the mining demo data. The database should contain both metadata, such as information about the mines, trucks, and their locations, as well as metric or process data, such as the amount mined per hour and truck utilization.
...
- **Get Current Value query**: Given an input or set of input parameters, returns the most current value and timestamp not in the future.
```SQL
...
order by Timestamp desc;
```
- **Get Trend Data Query**: Given a specific range of input parameters, including a start and end timestamp,returns a set of values and timestamps that fall within this range. To ensure that there is no empty space at the beginning or end of the trend, it's recommended to also return the previous and following value/timestamp pair.
```SQL
...
Now that you have expanded your mining empire and set up groups and KPIs to monitor it, it's time to enhance your visualizations. Visual KPI offers a range of chart options, including bar charts, managed trends, Pareto charts, Gantt charts, pie charts, XY plots, tables, and more. You can create these visualizations using the SQL remote context server.
...
- `GetBarChartRelationships`: Retrieves each bar chart's MetaID from the `GetBarChart` query and associate it with the mine MetaIDs from the `GetGroups` query. This process is similar to that of `GetKPIRelationships`. As a result, the Group with the MetaID of M01 will now include a bar chart with the MetaID: **M01_BarChart**.

- `GetTrends`:  Returns 10 trends (1 per mine) comparing Production Rate and Efficiency metrics.
- `GetTrendRelationships`: Retrieves each bar chart's MetaID from the `GetTrends` query and associate it with the mine MetaIDs from the `GetGroups` query.
...
- `GetWidgets`: When creating a widget, it's important to specify its position and size. Additionally, the widget should be linked to a specific item. For instance, if we want to create a KPI block, we need to provide the ObjectMetaID of the KPI that we want to display. Some widget types offer a variety of options. For example, KPI Blocks can be configured to hide their sparklines. Conversely, some widget types don't offer any options at all.
...
3. Use the drop-down menu to select the Mines profile group and Mines profile.
4. Click **Select**.

You can access other dashboards for each mine by using the dashboard drop-down menu.

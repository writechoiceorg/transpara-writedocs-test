...
The next steps walk you through creating a working SQL remote context server and installing the sample data. Before all is done, you will get to add more data, more visualizations and dashboards. Then yoi will be able to take a look at your data live in a Visual KPI site.
...
:::caution Important
During installation, please uncheck Manufacturing Demo under Install prebuilt demos and sample data. We do not want other demo data cluttering our Mining demo.
:::
...
:::tip
This setting can also be set as an override per individual KPI, but for our exercise it is simpler to change the global setting.
:::
...
:::note
Items under the gear icon are not modifiable via the RCS at the time of this document's creation. We do plan to address this in a future build.
:::
...
:::support 
If you fail to verify and the Connect String is not correct, then there is problems with security on the database. Step 2 talks about this, but if you are still stuck then please [contact our Support](https://www.transpara.com/contact-us/) and we will help you out.
:::
...
Now you will install a SQL Remote Context Server and dig in and understand the queries needed. Don't worry, we have provided these queries for you.
...
- **Pre & Post Process Queries**: These queries can be helpful for manipulating metadata. They can be run before or after other queries to process data or perform ETL tasks. They are useful for handling any data manipulation required on the metadata.


- **Error Handling**: This section explains how the RCS handles queries that don't return any data. Whenever the RCS is executed, a MetaID is used to map the objects in the foreign system (i.e., the data stored in the MiningData database) to the objects in Visual KPI (such as Groups, KPIs, Charts, etc.).
...
By default, the RCS updates constantly. As a result, any modifications you make in Visual KPI Designer will be lost when the SQL RCS runs next time. Keep in mind that the MiningData database is now the master, and we are simply following its lead.
...
If you have not stopped the RCS and it has been running continuously, then please take a moment to check your website. As you may have guessed, new groups have been created for the mines and trucks, along with new KPIs, using the same definition and relationship queries that we used before. 

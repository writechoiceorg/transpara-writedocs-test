...
To ensure accurate data representation on the KPI dashboard, it is recommended to modify the default setting for **KPI Stale Age**. The default setting is 60 minutes, which implies that when data retrieved via an Interface (such as truck or mine metrics) is older than an hour (timestamp), the KPI status will change to Stale. Therefore, it is advisable to adjust this setting to better suit your data requirements and ensure that the KPIs reflect the latest data.
...
In this step, it is recommended to use SQL Server Management Studio to create a database for the mining demo data. The database should contain both metadata, such as information about the mines, trucks, and their locations, as well as metric or process data, such as the amount mined per hour and truck utilization.

...
You can use ODBC interfaces to connect to an SQL database. In this section, you will learn how to install and configure ODBC interfaces using Visual KPI Server Manager. 
...
ODBC interfaces include a Web service and two SQL scripts (GetCurrentData.sql and GetTrendData.sql). Each SQL script uses four fields, two of which you will use to retrieve information in your KPIs:

- **LookUp**: The tag name.
- **Value**: The value associated with the tag name.

The other two fields are also used to return information but do not require your interaction:
...
The ODBC interface comes with three queries, including an optional one, that require configuration. Upon installation of an ODBC interface, example queries are automatically added. You'll need to edit these queries to fit your requirements.

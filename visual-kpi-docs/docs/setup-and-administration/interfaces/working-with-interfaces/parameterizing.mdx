...
When creating a query, you can pass parameters into it using index placeholders like `{0}`, `{1}`, and `{8}`. It is important to ensure that all queries have the same number of input parameters (i.e., `{0}` and `{1}`), and that the Historical Value query includes the `{TIMESTAMP}` parameter, while the Trend query includes both `{STARTDATE}` and `{ENDDATE}` parameters.
...
<Tabs>
  <TabItem value="Without Parameters" label="Without Parameters" default>
    ```sql

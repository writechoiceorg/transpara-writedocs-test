...
To save a Table, place an **x** in the first column and click the Save icon ( add icon ) in the open Tables tab of the Objects Toolbar. it's important to note that ALL Tables with an **x** in the (x) column will be saved. You can remove the **x** for any Table you don't want to save or update.
...
Tables with an assigned Display Order are listed at the top of the page, in Display Order. Tables that don't have an assigned Display Order, are displayed in alphabetical order, following the Tables with a Display Order.

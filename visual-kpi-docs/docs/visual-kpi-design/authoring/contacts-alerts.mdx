...
If some of the available parameters aren't clear, you can see the [Contacts](/docs/reference/visual-kpi-designer/site-settings/contacts) reference page.
...
Alerts in Visual KPI Designer allow you to define notification settings based on various states of KPIs. In Visual KPI, you aren't creating Alerts, you're creating Alert Templates, a predefined set of rules that determine when and how notifications are triggered based on KPI status. End users can select and apply these templates to the desired KPIs. 
...
On the **Alert Templates** worksheet, you find all existing templates. To add a new template to the list, follow the steps:
...
The Alert Templates are defined for the existing KPI status, which refers to the different statuses that a KPI can have, such as Low, High, Out of Range. The notifications will be triggered when the KPI enters or remains within a specified status for a certain period. 

...
View the trend in Single Scale mode with only the Target line indicator showing, or you can view in Multi-Scale mode that shows all status indicator line parameters. This is most appropriate when viewing managed or Ad Hoc trends with multiple pens with very different scales. On multi-scale trends, clicking on the pen name in the trend legend will adjust the Y-axis.

...
The trend line is amplified when you hover over a trend line or its legend name. This is especially useful when dealing with several pens on the same tread.
...
The trend scale feature enables you to use a single or multi-scale in a trend. The multi-scale option is most appropriate when viewing managed or Ad Hoc trends with multiple pens with very different scales. Clicking on the pen name in the trend legend when using multi-scale trends will adjust the Y-axis.
...
From any KPI list view, click the **Select KPI** icon (<SvgFont icon="255"/>) and select multiple KPIs by clicking the box to the left of any KPI in the list. From the sub-menu, select the trend icon or type of chart to view. 
...
If you are interested in deep-diving into trends, check out the [KPI Visualization](/docs/end-user/features/visualizations/kpi) page to learn about all the KPI visualization options that Visual KPI has to offer.
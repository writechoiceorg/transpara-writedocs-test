...
Pie Charts (<SvgFont icon="124"/>) display a simple graphic that shows the relative proportions of your compared assets. However, Pie Charts has some disavantages for Visual KPI, such as its size for screen presentation and the limited ability to compare peer data. 
...
XY Plots (<SvgFont icon="133"/>), or Scatter Charts as they are sometimes called, show the relationships between two sets of data. They are great for showing correlations between one or many variables or calculations. 

...
When you access an XY plot, you can add a fit line. The fit line presents the average value of Y across X. Note that the fit line doesn't necessarily present the average value across time since your X-axis doesn't need to be time. It's a valuable tool to identify trends.

To add the fit line, click **XY Plot Options** icon (<SvgFont icon="188 "/>) and select **Fit Line** or **Y Data + Fit Line** at the **Plot Contents**.
...
- From the total data points available for the 4 hours time range, 21 are within the considered pressure interval.

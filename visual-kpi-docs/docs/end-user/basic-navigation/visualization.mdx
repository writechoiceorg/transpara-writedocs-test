...
in the example below, the **Only Show Child Groups** (<SvgFont icon="207"/>) option is active. Thus, only the groups at the top of the Hierarchy are available and the **Management** is one of them. When the **Show all Descendant Groups** (<SvgFont icon="208"/>) is selected, subgroups show up. You can check the level of the subgroup by the number of **-** before the name the group has. Thus, you can verify that the **Management** group is composed of four other subgroups (Economic, Environmental, Operation, and Social).
...
The Group Map Table (<SvgFont icon="21"/>) view is similar to the [Group Map](visualization#view-as-group-map) view but has a different layout. It shows groups as titles in a table. As happens when using the [Group Map](visualization#view-as-group-map), you can select and click on any KPI in the Group Map Table to display details and other KPI options.
